...
@Component({
  ...
  styleUrls: ['./animal-management.component.scss']
})
  ...
  ngOnInit() {
    this.userDataSub = this.userService.userData.subscribe((data) => {
      if (data) {
        ...
        this.farms = data.farms ? data.farms : [];
        this.animals = data.animals.filter(animal => animal !== null);
        this.heatEvents = data.heatEvents ? data.heatEvents : [];
        this.healthEvents = data.healthEvents ? data.healthEvents : [];
