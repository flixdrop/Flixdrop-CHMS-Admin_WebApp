...
export class IllnessDetailsComponent implements OnInit, OnDestroy {
  userRole: string = "";
    ...
    this.userDataSub = this.userService.userData
      .pipe(
        ...
        map((data) => {
          ...
          this.healthEvents = data["healthEvents"].filter((item) => {
  ...
  handleInput(event: any) {
    ...
    if (!this.healthEvents || this.healthEvents.length === 0) {
      this.results = [];
      return;
    }

    this.results = this.healthEvents.filter((item) => {
  ...
  onClickNotify(illnessAlert: any) {
    console.log("illnessAlert- ", illnessAlert);
  }
}
